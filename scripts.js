const geoServerUrl = "/geoserver/ne/wfs?service=WFS&version=1.0.0&request=GetFeature&typeName=ne:repair_services&outputFormat=application/json";
...
function fetchWardsAndTypes() {
    fetch(`${proxyUrl}${apiUrl}?service=WFS&version=1.0.0&request=GetFeature&typeName=ne:repair_services&outputFormat=application/json`,{
        headers: {
            ...
            'Origin': 'https://nimishachaudhary.github.io', // Add the required origin
            'X-Requested-With': 'XMLHttpRequest', // Add the requested header
          },
        method: 'GET',
        //mode: 'no-cors',
      }).then((response) => {
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);S
            }
